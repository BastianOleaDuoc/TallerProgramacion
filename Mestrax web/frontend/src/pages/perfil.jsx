  ...
  return (
    <main className="container py-5 page-shell">
      ...
      <div className="card glass-panel p-4 border-0 mx-auto" style={{ maxWidth: '640px' }}>
        <div className="text-center mb-4">
          <img src={new URL('../img/unnamed.png', import.meta.url).href} alt="Avatar Usuario" className="rounded-circle border border-primary border-3" style={{ width: '120px', height: '120px', objectFit: 'cover' }} onError={(e) => { e.target.src = 'https://ui-avatars.com/api/?name=Usuario&background=3b82f6&color=fff&size=120'; }} />
        </div>
        <div id="perfilContent">
          <div className="mb-4 text-center">
            <h3 className="fw-bold text-white mb-1">Información personal</h3>
            <p className="text-muted small mb-0">Bienvenido, {usuarioActual.nombre || usuarioActual.email.split('@')[0]}.</p>
          </div>

          <div className="mb-3">
            <label className="form-label text-white">Nombre</label>
            <p className="fw-bold text-white mb-0">{usuarioActual.nombre || 'Usuario'}</p>
          </div>

          <div className="mb-3">
            <label className="form-label text-white">Correo electrónico</label>
            <p className="fw-bold text-white mb-0">{usuarioActual.email}</p>
          </div>

          <div className="mb-4">
            <label className="form-label text-white">Miembro desde</label>
            <p className="fw-bold text-white mb-0">{fechaRegistro}</p>
          </div>

          <button className="btn btn-danger w-100 mb-2" onClick={cerrarSesion}>Cerrar sesión</button>
