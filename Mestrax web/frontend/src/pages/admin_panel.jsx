...
export default function AdminPanel() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      const [productosRes, reservasRes, ventasRes] = await Promise.all([
        fetch(`${API_BASE}/productos`),
        fetch(`${API_BASE}/reservas`),
        fetch(`${API_BASE}/ventas`),
      ]);

      if (!productosRes.ok || !reservasRes.ok || !ventasRes.ok) {
        throw new Error('No se pudieron cargar los datos del panel');
      }

      const [productosData, reservasData, ventasData] = await Promise.all([
        productosRes.json(),
        reservasRes.json(),
        ventasRes.json(),
      ]);
  ...
  const actividadReciente = useMemo(() => {
    ...
    return [...reservasItems, ...ventasItems].slice(-5).reverse();
  }, [reservas, ventas]);
